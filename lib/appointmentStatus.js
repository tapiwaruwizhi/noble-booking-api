...
// Finds the status ID whose label contains `keyword` (case-insensitive) —
// e.g. findStatusIdByKeyword(map, "cancel") to get the "Cancelled" status ID
// without hardcoding it (IDs vary per ezyVet instance).
export function findStatusIdByKeyword(statusMap, keyword) {
  const kw = keyword.toLowerCase();
  for (const [id, name] of Object.entries(statusMap)) {
    if (String(name).toLowerCase().includes(kw)) return id;
  }
  return null;
}