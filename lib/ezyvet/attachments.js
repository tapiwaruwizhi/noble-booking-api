...
/**
 * Convenience: get the most recent attachment for an animal, treated as
 * its photo. We no longer filter strictly by a content_type/file_type
 * field — ezyVet's attachment list response doesn't reliably include one,
 * so that filter was silently returning nothing even when attachments
 * existed. Since this function is only ever used for animal photos, the
 * most recently added attachment is a reasonable stand-in.
 */
export async function getAnimalPhotoAttachment(animalId) {
  const attachments = await listAnimalAttachments(animalId);
  if (attachments.length === 0) return null;

  const sorted = [...attachments].sort((a, b) => (b.created_at ?? b.id ?? 0) - (a.created_at ?? a.id ?? 0));
  console.log("[attachments] Using most recent attachment as photo for animal", animalId, ":", JSON.stringify(sorted[0]));
  return sorted[0];
}